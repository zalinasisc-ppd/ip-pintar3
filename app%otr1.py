...
def compute_ipmp(df: pd.DataFrame) -> pd.DataFrame:
    ...
    needed = ["Mata Pelajaran", "Bil. Ambil", "% L PPT", "% L OTR1"]
    for c in needed:
        if c not in df.columns:
            raise ValueError(f"Kolum '{c}' tiada dalam data.")
    # Coerce numerics
    for c in ["Bil. Daftar", "Bil. Ambil", "% L PPT", "% L OTR1"]:
        if c in df.columns:
...
st.sidebar.markdown("---")
st.sidebar.caption("Kolum diperlukan: Mata Pelajaran, Bil. Ambil, % L PPT, % L OTR1. (Bil. Daftar opsyenal).")
...
st.subheader("Perbandingan GPMP (OTR1 vs PPT)")
long_df = df_result.melt(id_vars=["Mata Pelajaran"], value_vars=["% L OTR1", "% L PPT"],
                         var_name="Jenis", value_name="GPMP")
fig2 = px.line(long_df, x="Mata Pelajaran", y="GPMP", color="Jenis",
               markers=True, title="GPMP Lepas vs Semasa Mengikut Subjek")
